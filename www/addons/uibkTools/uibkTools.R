  ...
  output$uibkToolsDl <- downloadHandler(
    ...
    content = function(fname) {
      ...
      if(uibkToolsData()$contentType=="application/zip") {
        #todo: create zip from files
        #   zip(zipfile=fname, files=file, flags='-r9XjFS')
...
createRexParticipantsList = function(args) {
  ...
  return(list(name=name, files=NULL, data=data, contentType=contentType))
}
...
createGradingLists = function(args) {
  ...
  gradingData = lapply(args$visGradingLists, \(x){
    content = read.table(text=x[[3]], sep=";", header = FALSE)
  ...
  return(list(name=name, files=sapply(args$visGradingLists, \(x) paste0(x[1:2], collapse=".")), data=NULL, contentType=contentType))
}
