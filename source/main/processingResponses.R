...
examParseResponse = function(session, exerciseData, error) {
	with(exerciseData, {
	  ...
	  if(!error){
  		...
  		session$sendCustomMessage("setExerciseConvert", rjs_keyValuePairsToJsonObject(c("id", "value"), c(id, convert)))
  		session$sendCustomMessage("setExerciseRmdExport", rjs_keyValuePairsToJsonObject(c("id", "value"), c(id, rmdExport)))
